...
export class TreeSimplex {
    ...
    public getValueAt(x: number, y: number) {
        let stretchOffset = (x + y) * TreeSimplex.STRETCH_CONSTANT_2D;
        let xs = x + stretchOffset;
        let ys = y + stretchOffset;
        let xsb = math.floor(xs);
        let ysb = math.floor(ys);
        let squishOffset = (xsb + ysb) * TreeSimplex.SQUISH_CONSTANT_2D;
        ...
        let value = 0;
        //Contribution (1,0)
        let dx1 = dx0 - 1 - TreeSimplex.SQUISH_CONSTANT_2D;
        let dy1 = dy0 - TreeSimplex.SQUISH_CONSTANT_2D;
        ...
        //Contribution (0,1)
        let dx2 = dx0 - TreeSimplex.SQUISH_CONSTANT_2D;
        let dy2 = dy0 - 1 - TreeSimplex.SQUISH_CONSTANT_2D;
        ...
        if (inSum <= 1) { //We're inside the triangle (2-Simplex) at (0,0)
            ...
            if (zins > xins || zins > yins) { //(0,0) is one of the closest two triangular vertices
                ...
                xsv_ext = xsb + 1;
                ysv_ext = ysb + 1;
                dx_ext = dx0 - 1 - 2 * TreeSimplex.SQUISH_CONSTANT_2D;
                dy_ext = dy0 - 1 - 2 * TreeSimplex.SQUISH_CONSTANT_2D;
            }
            ...
            if (zins < xins || zins < yins) { //(0,0) is one of the closest two triangular vertices
                if (xins > yins) {
                    xsv_ext = xsb + 2;
                    ysv_ext = ysb;
                    dx_ext = dx0 - 2 - 2 * TreeSimplex.SQUISH_CONSTANT_2D;
                    dy_ext = dy0 - 2 * TreeSimplex.SQUISH_CONSTANT_2D;
                } else {
                    xsv_ext = xsb;
                    ysv_ext = ysb + 2;
                    dx_ext = dx0 - 2 * TreeSimplex.SQUISH_CONSTANT_2D;
                    dy_ext = dy0 - 2 - 2 * TreeSimplex.SQUISH_CONSTANT_2D;
                }
            } else { //(1,0) and (0,1) are the closest two vertices.
                dx_ext = dx0;
                dy_ext = dy0;
            ...
            xsb += 1;
            ysb += 1;
            dx0 = dx0 - 1 - 2 * TreeSimplex.SQUISH_CONSTANT_2D;
            dy0 = dy0 - 1 - 2 * TreeSimplex.SQUISH_CONSTANT_2D;
        }
        ...
        return value / TreeSimplex.NORM_CONSTANT_2D;
    }

    private extrapolate(xsb: number, ysb: number, dx: number, dy: number) {
        let index = this.perm[(this.perm[xsb & 0xFF] + ysb) & 0xFF] & 0x0E;
        return TreeSimplex.gradients2D[index] * dx + TreeSimplex.gradients2D[index + 1] * dy;
    }
